...
import { iRescueHistoryRequest, iRescueHistoryResponse, iUniqueRescueHistoryRequest } from "../interfaces/rescueHistory.interfaces";
...
const listRescueHistoryForPubController = async (req: Request, res: Response): Promise<Response> => {
  ...
  const rescueHistoryData: iUniqueRescueHistoryRequest = req.body;
  const registerClient = await listRescueHistoryForPubService(rescueHistoryData, pubId);
